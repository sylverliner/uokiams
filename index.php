...
if(isset($_POST["btn_signin"])){
  $login_reg_number = $_POST["txtusername"];
  $login_password = $_POST["txtpassword"];

  $check_user_existence = "SELECT * FROM registered_students WHERE reg_number='$login_reg_number' AND password='$login_password' LIMIT 1";
  $run_query = mysqli_query($conn,$check_user_existence);
  $user_exist = mysqli_num_rows($run_query);
  if($user_exist==1){
    $get_user_details = "SELECT * FROM registered_students WHERE reg_number='$login_reg_number'";
    ...
    setcookie("student_reg_number",$login_reg_number,time() + (86400 * 30),"/");
...
if(isset($_POST["btn_signup"])){
  ...
  $reg_reg_number = $_POST["txt_signup_indexnumber"];
  $reg_password = $_POST["txt_signup_password"];

  if($reg_first_name !="" && $reg_last_name!="" && $reg_reg_number!="" && $reg_password!=""){
  $check_existence = "SELECT * FROM registered_students WHERE reg_number='$reg_reg_number'";
  ...
  if($data_existence==1){
    ...
    $register_student_query = "INSERT INTO registered_students (first_name,last_name,reg_number,password) VALUES ('$reg_first_name','$reg_last_name','$reg_reg_number','$reg_password')";
    if($execute_register_student = mysqli_query($conn,$register_student_query)){
      ...
      setcookie("student_reg_number",$reg_reg_number,time() + (86400 * 30),"/");
...
<div class="login-wrap">
	<div class="login-html">
		...
		<div id="text_holder">
			<h2><span style="color:rgba(255, 152, 0, 0.91);text-align:center;font-family:sans">UNIVERSITY of KABIANGA <br><br>INDUSTRIAL ATTACHMENT <br><br>MANAGEMENT SYSTEM</span></h2>
		</div>
		<input id="tab-1" type="radio" name="tab" class="sign-in" checked><label for="tab-1" class="tab" style="margin-left: 100px;">Sign In</label>
      ...
      <form method="post" action="">
          ...
          <a href="password-reset/forgot_password.php" style="float: right; color:#26e2f7;">Forgot Password?</a>
				</div>
				<div class="group">
    ...
    <form method="post" action="">
        ...
        <div class="group">
          ...
          <input id="reg_number" type="text" class="input" name="txt_signup_indexnumber">
        </div>
				<div class="group">
					<label for="pass" class="label">Password</label>
