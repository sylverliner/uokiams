...
if(isset($_POST["btn_search"])){
  ...
  if($search_by!=""&& $search_term!=""){
    ...
    switch ($search_by) {
      ...
      $mysql_query_1 = "SELECT * FROM industrial_registration WHERE first_name LIKE '%$search_term%'";

      break;

      case 'Last Name':

      $mysql_query_1 = "SELECT * FROM industrial_registration WHERE last_name LIKE '%$search_term%'";

      break;

    case 'Reg Number':

      $mysql_query_1 = "SELECT * FROM industrial_registration WHERE reg_number LIKE '%$search_term%'";

        break;

      case 'School':

      $mysql_query_1 = "SELECT * FROM industrial_registration WHERE school LIKE '%$search_term%'";

        break;

      case 'Department':

      $mysql_query_1 = "SELECT * FROM industrial_registration WHERE student_department LIKE '%$search_term%'";

        break; 

      case 'Course':

      $mysql_query_1 = "SELECT * FROM industrial_registration WHERE course LIKE '%$search_term%'";
    
        break;

      case 'Level':

      $mysql_query_1 = "SELECT * FROM industrial_registration WHERE level LIKE '%$search_term%'";
...
<div id="student_name"><span style="color:rgb(255, 198, 0);font-size:1.1em"><em>Welcome,</em>&nbsp; </span><span style="font-family:serif"><?php echo "Coordinator"?></span></div>
</div>
